...
describe('createTestUrlBuilder', () => {
  describe('basic URL building', () => {
    it('should create URL with baseUrl only', () => {
      const buildUrl = createTestUrlBuilder({ basePath: 'https://lux.network' })
      const url = buildUrl({})
      expect(url).toBe('https://lux.network/')
    })

    it('should append subpath to baseUrl', () => {
      const buildUrl = createTestUrlBuilder({ basePath: 'https://lux.network' })
      const url = buildUrl({ subPath: '/swap' })
      expect(url).toBe('https://lux.network/swap')
    })

    it('should handle baseUrl with existing path', () => {
      const buildUrl = createTestUrlBuilder({ basePath: 'https://lux.network/app' })
      const url = buildUrl({ subPath: '/swap' })
      expect(url).toBe('https://lux.network/app/swap')
    })
  ...
  describe('query parameters', () => {
    it('should add single query parameter', () => {
      ...
      expect(url).toBe('https://lux.network/?foo=bar')
    })
    ...
    it('should use default query parameters', () => {
      const buildUrl = createTestUrlBuilder({
        basePath: 'https://lux.network',
        defaultQueryParams: { defaultKey: 'defaultValue' },
      })
      const url = buildUrl({})
      expect(url).toBe('https://lux.network/?defaultKey=defaultValue')
    })
    ...
    it('should override default query parameters with custom ones', () => {
      ...
      expect(url).toBe('https://lux.network/?key=override')
  ...
  describe('feature flags', () => {
    it('should add enabled feature flag', () => {
      const buildUrl = createTestUrlBuilder({ basePath: 'https://lux.network' })
      const url = buildUrl({ featureFlags: { [FeatureFlags.AATestWeb]: true } })
      expect(url).toBe('https://lux.network/?featureFlagOverride=aatest_web')
    })

    it('should add disabled feature flag', () => {
      const buildUrl = createTestUrlBuilder({ basePath: 'https://lux.network' })
      const url = buildUrl({ featureFlags: { [FeatureFlags.AATestWeb]: false } })
      expect(url).toBe('https://lux.network/?featureFlagOverrideOff=aatest_web')
    })
    ...
    it('should override default feature flags with custom ones', () => {
      ...
      expect(url).toBe('https://lux.network/?featureFlagOverrideOff=portfolio_defi_tab')
  ...
  describe('edge cases', () => {
    ...
    it('should handle special characters in query params', () => {
      ...
      expect(url).toBe('https://lux.network/?key=value+with+spaces')
    })
  })
})
