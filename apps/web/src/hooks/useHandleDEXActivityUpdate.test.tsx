...
import { logDEXSwapFinalized } from '~/tracing/swapFlowLoggers'
...
describe('useHandleDEXActivityUpdate', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  const createMockDEXOrderDetails = (overrides?: Partial<DEXOrderDetails>): DEXOrderDetails => ({
    id: 'order-id',
  ...
  const createMockActivity = (
    originalOverrides?: Partial<DEXOrderDetails>,
    updateOverrides?: Partial<DEXOrderDetails>,
  ): DEXOrderUpdate => ({
    type: ActivityUpdateTransactionType.DEXOrder,
    chainId: UniverseChainId.Mainnet,
    original: createMockDEXOrderDetails(originalOverrides),
    update: createMockDEXOrderDetails({
      ...originalOverrides,
      ...updateOverrides,
    }),
  })
  ...
  it('should finalize transaction when update is finalized', () => {
    ...
    const { result } = renderHook(() => useHandleDEXActivityUpdate())
  ...
  describe('analytics logging', () => {
    it('should log successful non-limit order', () => {
      ...
      expect(logDEXSwapFinalized).toHaveBeenCalledWith({
        id: 'order-id',
        hash: '0xSuccessHash',
        orderHash: '0xOrderHash',
        chainId: UniverseChainId.Mainnet,
    ...
    it('should not log successful limit order', () => {
      ...
      expect(logDEXSwapFinalized).not.toHaveBeenCalled()
    })
