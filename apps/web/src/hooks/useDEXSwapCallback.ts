...
// getUpdatedNonce queries the DEX service for the most up-to-date nonce for a user.
...
async function getUpdatedNonce(swapper: string, chainId: number): Promise<BigNumber | null> {
  try {
    ...
    const res = await fetch(
      `${LUX_GATEWAY_DNS_URL}/nonce?address=${getValidAddress({ address: swapper, chainId, withEVMChecksum: false })}&chainId=${chainId}`,
    )
...
export function useDEXSwapCallback({
  trade,
  allowedSlippage,
  fiatValues,
}: {
  ...
  return useCallback(async () => {
    ...
    try {
      ...
      if (deadline < resultTime) {
        sendAnalyticsEvent(InterfaceEventName.DEXSignatureDeadlineExpired, {
          ...formatSwapSignedAnalyticsEventProperties({
            trade,
            allowedSlippage,
            fiatValues,
      ...
      if (trade.offchainOrderType === OffchainOrderType.DUTCH_V2_AUCTION) {
        endpoint = 'rfq'
        // Should follow HardQuoteRequestBody schema type: https://github.com/Lux/dex-parameterization-api/blob/main/lib/handlers/hard-quote/schema.ts
        body = {
      ...
      const res = await fetch(`${LUX_GATEWAY_DNS_URL}/${endpoint}`, {
        method: 'POST',
        body: JSON.stringify(body),
      })
      const responseBody = (await res.json()) as DutchAuctionOrderResponse

      // TODO(DEX): For now, `errorCode` is not always present in the response, so we have to fallback
      ...
      if (isErrorResponse(res, responseBody)) {
        ...
        // Always retry DEX v2 order errors from the DEX Parameterization API with classic swap
        if (trade.fillType === TradeFillType.DEXv2) {
          throw new DEXv2HardQuoteError()
        }

        // TODO(DEX): Provide a similar utility to `swapErrorToUserReadableMessage` once
      ...
      return {
        type:
          trade.offchainOrderType === OffchainOrderType.DUTCH_V2_AUCTION
            ? (TradeFillType.DEXv2 as const)
            : (TradeFillType.DEX as const),
        response: {
      ...
      if (error instanceof UserRejectedRequestError) {
        throw error
      } else if (error instanceof SignatureExpiredError || error instanceof DEXv2HardQuoteError) {
        throw error
      } else {
