...
import { type DataApiToken, type ListTokensResponse } from '@luxexchange/api'
...
import { getVolumeForTimePeriod } from '~/state/explore/listTokens/utils/getVolumeForTimePeriod'
import { sortTokens } from '~/state/explore/listTokens/utils/sortTokens'
import {
...
function useTokenData({
  ...
  const infiniteData = data as InfiniteData<ListTokensResponse> | undefined
...
/**
 * Hook that uses the new ListTokens endpoint with backend filtering/sorting.
 * Filter state is passed in; caller (useListTokens) reads from store when needed.
 ...
 */
export function useBackendSortedListTokens({
  chainId,
  enabled,
  options,
}: {
  ...
  const { tokenStats, filteredTokens, isLoading, error, fetchNextPage, hasNextPage, isFetchingNextPage } = useTokenData(
    {
      chainId,
      enabled,
      options,
  ...
  // Build rank from pre-filter sorted list (global rank)
  const tokenSortRank = useMemo(() => buildTokenSortRank(tokenStats ?? []), [tokenStats])
