...
import type { LXInterfaceMulticall } from './abi/types'
...
interface ReturnDataItem {
  gasUsed: any; // Replace 'any' with the correct type, e.g., BigNumber or number.
  returnData: any; // Replace 'any' with the correct type.
  success: boolean;
}
...
async function fetchChunk(
  ...
  try {
    ...
    if (isDebug) {
      returnData.forEach(({ gasUsed, returnData, success }: ReturnDataItem, i: number) => {
        if (
          !success &&
          returnData.length === 2 &&
...
function onFetchChunkSuccess(
  ...
  console.log("onFetchChunkSuccess: ", results);
...
function Updater(props: UpdaterProps): null {
  ...
  const cancellations = useRef<{ blockNumber: number; cancellations: (() => void)[] } | undefined>(undefined)
