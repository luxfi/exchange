...
class PermissionedTokenValidator {
    ...
    static isPermissionedToken(tokenAddress, chainId) {
        return constants_1.PERMISSIONED_TOKENS.some(token => token.address.toLowerCase() === tokenAddress.toLowerCase() && token.chainId === chainId);
    }
    /**
    ...
    static preTransferCheck(provider, tokenAddress, from, to, value) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const token = constants_1.PERMISSIONED_TOKENS.find(token => token.address.toLowerCase() === tokenAddress.toLowerCase());
            ...
            if (token.usesProxy) {
                ...
                tokenContract = contracts_1.DSTokenInterface__factory.connect(targetAddress, provider);
            }
            else {
                tokenContract = contracts_1.DSTokenInterface__factory.connect(tokenAddress, provider);
            }
