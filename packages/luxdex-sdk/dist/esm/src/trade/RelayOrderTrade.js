...
export class RelayOrderTrade {
    ...
    getFeeInputStartEndAmounts() {
        ...
        const currencyIn = this._currenciesIn.find((currency) => areCurrenciesEqual(currency, this.order.info.fee.token, currency.chainId));
        if (!currencyIn) {
            throw new Error("currency output from order must exist in currenciesOut list");
        }
        const startEndAmounts = {
            startAmount: CurrencyAmount.fromRawAmount(currencyIn, this.order.info.fee.startAmount.toString()),
            endAmount: CurrencyAmount.fromRawAmount(currencyIn, this.order.info.fee.endAmount.toString()),
        };
    ...
    getInputAmount() {
        ...
        // assume single chain ids across all outputs for now
        const currencyIn = this._currenciesIn.find((currency) => areCurrenciesEqual(currency, this.order.info.input.token, currency.chainId));
        if (!currencyIn) {
            throw new Error("currency input from order must exist in currenciesIn list");
        }
        const inputAmount = CurrencyAmount.fromRawAmount(currencyIn, this.order.info.input.amount.toString());
    ...
    get executionPrice() {
        var _a;
        return ((_a = this._executionPrice) !== null && _a !== void 0 ? _a : (this._executionPrice = new Price(this.amountIn.currency, this.outputAmount.currency, this.amountIn.quotient, this.outputAmount.quotient)));
    }
    /**
     ...
     */
    worstExecutionPrice() {
        return new Price(this.amountIn.currency, this.outputAmount.currency, this.amountIn.quotient, this.outputAmount.quotient);
    }
}
