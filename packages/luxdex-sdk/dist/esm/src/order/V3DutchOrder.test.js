...
describe("V3DutchOrder", () => {
    ...
    const getFullOrderInfoWithoutOverrides = Object.assign(Object.assign({}, getFullOrderInfo({})), { cosignerData: COSIGNER_DATA_WITHOUT_OVERRIDES });
    ...
    it("parses inner v3 order with no cosigner overrides, both input and output curves", () => {
        const orderInfoJSON = Object.assign(Object.assign({}, getFullOrderInfo({})), { nonce: "21", startingBaseFee: "0", input: {
                token: INPUT_TOKEN,
                startAmount: "1000000",
                curve: {
                    relativeBlocks: [1, 2, 3, 4],
    ...
    it("valid signature over inner order", () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        expect(order.getSigner(signature)).equal(yield wallet.getAddress());
        const fullOrder = CosignedV3DutchOrder.fromUnsignedOrder(order, fullOrderInfo.cosignerData, fullOrderInfo.cosignature);
        expect(fullOrder.getSigner(signature)).equal(yield wallet.getAddress());
    }));
    it("validates cosignature over (hash || cosignerData)", () => __awaiter(void 0, void 0, void 0, function* () {
        const wallet = ethers.Wallet.createRandom();
        const orderInfo = getFullOrderInfo({
            cosigner: yield wallet.getAddress(),
        });
        const order = new UnsignedV3DutchOrder(orderInfo, 1);
        const fullOrderHash = order.cosignatureHash(orderInfo.cosignerData);
        const cosignature = ethers.utils.joinSignature(wallet._signingKey().signDigest(fullOrderHash));
        const signedOrder = CosignedV3DutchOrder.fromUnsignedOrder(order, COSIGNER_DATA_WITH_OVERRIDES, cosignature);
        expect(signedOrder.recoverCosigner()).equal(yield wallet.getAddress());
    }));
    describe("resolve DutchV3 orders", () => {
        ...
        it("resolves with original value when overrides==0", () => {
            const order = new CosignedV3DutchOrder(getFullOrderInfo({
        ...
        it("resolves when filler has exclusivity: Before Decay Start", () => {
            ...
            const order = new CosignedV3DutchOrder(getFullOrderInfo({
                cosignerData: Object.assign(Object.assign({}, COSIGNER_DATA_WITH_OVERRIDES), { exclusiveFiller }),
            }), CHAIN_ID);
            const resolved = order.resolve({
