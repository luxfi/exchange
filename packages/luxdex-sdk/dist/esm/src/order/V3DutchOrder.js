...
export class UnsignedV3DutchOrder {
    ...
    static fromJSON(json, chainId, _permit2Address) {
        return new UnsignedV3DutchOrder(Object.assign(Object.assign({}, json), { nonce: BigNumber.from(json.nonce), startingBaseFee: BigNumber.from(json.startingBaseFee), input: Object.assign(Object.assign({}, json.input), { startAmount: BigNumber.from(json.input.startAmount), curve: {
                    relativeBlocks: json.input.curve.relativeBlocks,
                    relativeAmounts: json.input.curve.relativeAmounts.map((amount) => BigInt(amount)),
                }, maxAmount: BigNumber.from(json.input.maxAmount), adjustmentPerGweiBaseFee: BigNumber.from(json.input.adjustmentPerGweiBaseFee) }), outputs: json.outputs.map((output) => (Object.assign(Object.assign({}, output), { startAmount: BigNumber.from(output.startAmount), curve: {
                    relativeBlocks: output.curve.relativeBlocks,
                    relativeAmounts: output.curve.relativeAmounts.map((amount) => BigInt(amount)),
                }, minAmount: BigNumber.from(output.minAmount), adjustmentPerGweiBaseFee: BigNumber.from(output.adjustmentPerGweiBaseFee) }))) }), chainId, _permit2Address);
    }
    /**
     * @inheritdoc order
     */
...
export class CosignedV3DutchOrder extends UnsignedV3DutchOrder {
    ...
    static fromJSON(json, chainId, _permit2Address) {
        return new CosignedV3DutchOrder(Object.assign(Object.assign({}, json), { nonce: BigNumber.from(json.nonce), startingBaseFee: BigNumber.from(json.startingBaseFee), input: {
                ...
                exclusivityOverrideBps: BigNumber.from(json.cosignerData.exclusivityOverrideBps),
...
export function encodeRelativeBlocks(relativeBlocks) {
