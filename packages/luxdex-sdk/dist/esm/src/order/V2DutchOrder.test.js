...
describe("V2DutchOrder", () => {
    ...
    it("parses the inner v2 order with no cosignerOverrides", () => {
        ...
        expect(order.info.outputs[0].startAmount).toEqual(BigNumber.from("1000000"));
    });
    it("valid signature over inner order", () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const fullOrder = CosignedV2DutchOrder.fromUnsignedOrder(order, fullOrderInfo.cosignerData, fullOrderInfo.cosignature);
        expect(fullOrder.getSigner(signature)).toEqual(yield wallet.getAddress());
    }));
    it("validates cosignature over (hash || cosignerData)", () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const cosignature = yield wallet.signMessage(fullOrderHash);
        const signedOrder = CosignedV2DutchOrder.fromUnsignedOrder(order, COSIGNER_DATA, cosignature);
        expect(signedOrder.recoverCosigner()).toEqual(yield wallet.getAddress());
    }));
    describe("resolve", () => {
        ...
        it("resolves with original value when overrides == 0", () => {
            const order = new CosignedV2DutchOrder(getFullOrderInfo({
                cosignerData: {
