...
describe("DutchOrderBuilder", () => {
    ...
    it("Regenerates builder from order json", () => {
        ...
        const regenerated = DutchOrderBuilder.fromOrder(DutchOrder.fromJSON(json, 1)).build();
        expect(regenerated.toJSON()).toMatchObject(order.toJSON());
    });
    ...
    it("Unknown chainId", () => {
        ...
        expect(() => new DutchOrderBuilder(chainId)).toThrow(`Missing configuration for reactor: ${chainId}`);
    });
    it("Must set swapper", () => {
        const deadline = Math.floor(Date.now() / 1000) + 1000;
        expect(() => builder
