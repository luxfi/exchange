...
export class RelayOrderBuilder {
    ...
    fee(fee) {
        invariant(fee.startAmount.lte(fee.endAmount), `startAmount must be less than or equal than endAmount: ${fee.startAmount.toString()}`);
        this.info.fee = fee;
        return this;
    }
    build() {
        ...
        invariant(this.info.deadline > Date.now() / 1000, `Deadline must be in the future: ${this.info.deadline}`);
        invariant(this.info.swapper !== undefined, "swapper not set");
        invariant(this.info.universalRouterCalldata !== undefined, "universalRouterCalldata not set");
        invariant(this.info.input !== undefined, "input not set");
        invariant(this.info.fee !== undefined, "fee not set");
        invariant(!this.info.deadline || this.info.fee.startTime <= this.info.deadline, `feeStartTime must be before or same as deadline: ${this.info.fee.startTime}`);
        invariant(!this.info.deadline || this.info.fee.endTime <= this.info.deadline, `feeEndTime must be before or same as deadline: ${this.info.fee.endTime}`);
        return new RelayOrder(Object.assign(this.info, {
