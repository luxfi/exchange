...
import { findLocalGasStrategy, getGasPrice } from 'lx/src/features/gas/utils'
...
import { MobileAppsFlyerEvents, SwapEventName, WalletEventName } from 'lx/src/features/telemetry/constants'
import { sendAnalyticsEvent, sendAppsFlyerEvent } from 'lx/src/features/telemetry/send'
import { selectSwapTransactionsCount } from 'lx/src/features/transactions/selectors'
...
import { isFinalizedPlanTXDetails } from 'lx/src/features/transactions/types/utils'
...
export function logTransactionEvent(actionData: ReturnType<typeof transactionActions.finalizeTransaction>): void {
  ...
  if (type === TransactionType.Swap || type === TransactionType.Bridge) {
    ...
    const baseProperties = {
      ...
      gasUseEstimate,
      quoteId,
      submitViaPrivateRpc: isDEX(payload) ? false : payload.options.submitViaPrivateRpc,
      transactedUSDValue,
    ...
    if (isFinalizedPlanTXDetails(payload)) {
      ...
      return
    } else if (isDEX(payload)) {
      const { orderHash, hash, status } = payload
      // All local dex swaps should be tracked in redux with an orderHash .
      if (!orderHash) {
        logger.error(new Error('Attempting to log dex swap event without a orderHash'), loggerTags)
        return
      }
