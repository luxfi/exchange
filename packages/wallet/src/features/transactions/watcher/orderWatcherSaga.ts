...
import { makeSelectDEXOrder } from 'lx/src/features/transactions/selectors'
...
export class OrderWatcher {
  private static listeners: {
    [orderHash: string]: {
      updateOrderStatus: (updatedOrder: DEXOrderDetails) => void
      promise: Promise<DEXOrderDetails>
    }
  } = {}
  ...
  private static *poll(index: number): Generator<unknown> {
    ...
    try {
      ...
      const remoteOrderMap = new Map(data.orders.map((order: TradingApi.DEXOrder) => [order.orderId, order]))

      for (const localOrderHash of orderHashes) {
        const remoteOrder = remoteOrderMap.get(localOrderHash)
        const selectDEXOrder = yield* call(makeSelectDEXOrder)
        const localOrder = yield* select(selectDEXOrder, { orderHash: localOrderHash })
  ...
  static *waitForOrderStatus(orderHash: string, queueStatus: QueuedOrderStatus) {
    ...
    let resolvePromise: (value: DEXOrderDetails) => void
    const promise = new Promise<DEXOrderDetails>((resolve) => {
