...
interface UseParseDEXSwapResult extends ParsedTransactionData {
  /** Whether the hook is still loading currency info */
  isLoading: boolean
}

/**
 * Hook to parse DEX swap typed data into TransactionSection format
 * compatible with TransactionPreviewCard display.
 *
 * @param typedData - The DEX swap request typed data, or null if not a DEX swap
 ...
 */
export function useParseDEXSwap(
  typedData: DEXSwapRequest | null,
  chainId: UniverseChainId,
): UseParseDEXSwapResult {
  ...
  const sections = useMemo((): TransactionSection[] => {
    // Return empty sections if not a DEX swap
