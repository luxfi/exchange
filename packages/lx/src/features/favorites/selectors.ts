...
const selectFavoriteTokensWithPossibleDuplicates = (state: LuxRootState): string[] => state.favorites.tokens
...
export const makeSelectHasTokenFavorited = (): Selector<LuxRootState, boolean, [string]> =>
  createSelector(
    selectFavoriteTokens,
    (_: LuxRootState, currencyId: string) => currencyId,
    (tokens, currencyId) => tokens.includes(normalizeCurrencyIdForMapLookup(currencyId)),
  )

const selectWatchedAddresses = (state: LuxRootState): string[] => state.favorites.watchedAddresses
