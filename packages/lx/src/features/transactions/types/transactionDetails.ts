...
export interface TransactionDetailsCore extends TransactionId {
   ...
   * - DEX: For DEX orders hash may be undefined for non-successful transactions,
   * as the dex backend does not provide hashes for cancelled, failed, or expired orders.
...
export interface DEXOrderExtension {
  routing:
  ...
  // DEX orders will also have a transaction `hash` if they become filled.
  ...
  // Contains the serialized/encoded DEX order data that gets submitted to the DEX system for execution.
  encodedOrder?: string

  // The Unix timestamp when the DEX order expires and can no longer be filled
...
export type DEXOrderDetails<TBase extends TransactionDetailsCore = WalletBaseTransactionDetails> = TBase &
  DEXOrderExtension
...
export interface WrapTransactionInfo extends BaseTransactionInfo {
  ...
  // Currently, this will only be set for wraps that are part of a DEX native-input swap.
  swapTxId?: string
}
