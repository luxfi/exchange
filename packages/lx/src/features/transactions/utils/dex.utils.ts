 ...
 * Used when creating transactions from external DEX orders.
 */
 ...
 * Limit orders are DEX orders with DUTCH_LIMIT routing.
 */
export function isLimitOrder(tx: TransactionDetails): tx is DEXOrderDetails {
  return isDEX(tx) && tx.routing === TradingApi.Routing.DUTCH_LIMIT
}

/**
 * Type guard to check if a DEX order has the encoded order data needed for cancellation.
 ...
 */
export function hasEncodedOrder(order: DEXOrderDetails): order is DEXOrderDetails & { encodedOrder: string } {
