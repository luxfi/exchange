  ...
  switch (data?.routing) {
    case TradingApi.Routing.CLASSIC: {
      ...
      return new ClassicTrade({
        quote: data,
        deadline,
        v2Routes: routes.flatMap((r) => (r.routev2 ? { ...r, routev2: r.routev2 } : [])) as any,
        v3Routes: routes.flatMap((r) => (r.routev3 ? { ...r, routev3: r.routev3 } : [])) as any,
        v4Routes: routes.flatMap((r) => (r.routev4 ? { ...r, routev4: r.routev4 } : [])) as any,
        mixedRoutes: routes.flatMap((r) => (r.mixedRoute ? { ...r, mixedRoute: r.mixedRoute } : [])) as any,
        tradeType,
      })
    }
...
// Lux chain IDs that we support for local/test trading (wrap/unwrap, DEX swaps)
const LUX_CHAIN_IDS: number[] = [
  1337, // LuxDev (local dev mode with instant finality)
...
function isTradingApiSupportedChainId(chainId?: number): chainId is TradingApi.ChainId {
  ...
  // Support both Trading API chains and Lux ecosystem chains
  return SUPPORTED_TRADING_API_CHAIN_IDS.includes(chainId) || LUX_CHAIN_IDS.includes(chainId)
}
...
export function toTradingApiSupportedChainId(chainId: Maybe<number>): TradingApi.ChainId | undefined {
  ...
  // Cast is safe here - for Lux chains we treat them as valid chain IDs for our mock API
