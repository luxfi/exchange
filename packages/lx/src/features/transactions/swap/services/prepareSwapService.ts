...
import { prefetchPlan } from 'lx/src/features/transactions/swap/plan/prefetchedPlanStore'
  ...
  function handleEventAction(action: ReviewAction): void {
    switch (action.type) {
      ...
      case ReviewActionType.SHOW_LOW_BALANCE:
        handleShowMaxNativeTransferModal()
        sendAnalyticsEvent(LuxEventName.LowNetworkTokenInfoModalOpened, action.payload)
        break
