...
import { getRouteAnalyticsData, logSwapQuoteFetch } from '@l.x/lx/src/features/transactions/swap/analytics'
import { ClassicTrade, Trade } from '@l.x/lx/src/features/transactions/swap/types/trade'

vi.mock('@l.x/lx/src/features/telemetry/send', () => ({
  sendAnalyticsEvent: vi.fn(),
}))

vi.mock('@l.x/lx/src/features/transactions/swap/utils/SwapEventTimestampTracker', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@l.x/lx/src/features/transactions/swap/utils/SwapEventTimestampTracker')>()
  return {
    ...actual,
    timestampTracker: {
...
// Mock the @luxamm/v2-sdk package to provide Pair.getAddress
vi.mock('@luxamm/v2-sdk', async (importOriginal) => {
  const originalModule = await importOriginal<typeof import('@luxamm/v2-sdk')>()
...
vi.mock('@luxamm/v3-sdk', async (importOriginal) => {
  const originalModule = await importOriginal<typeof import('@luxamm/v3-sdk')>()
...
const { Pair: V2Pool } = await vi.importActual<typeof import('@luxamm/v2-sdk')>('@luxamm/v2-sdk')
...
const { Pool: V3Pool } = await vi.importActual<typeof import('@luxamm/v3-sdk')>('@luxamm/v3-sdk')
Object.setPrototypeOf(mockV3Pool, V3Pool.prototype)
//
const mockV4Pool = { poolId: '0xpool1', v4: true }
const { Pool: V4Pool } = await vi.importActual<typeof import('@luxamm/v4-sdk')>('@luxamm/v4-sdk')
